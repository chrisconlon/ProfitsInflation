...
├── code/
│   ├── download_data.py          # [Python] Download all external data to raw_data/ (run once)
...
│   ├── Section1All_xls.xlsx      # BEA NIPA Section 1 (downloaded by download_data.py)
│   ├── Section6All_xls.xlsx      # BEA NIPA Section 6 (downloaded by download_data.py)
│   ├── GrossOutput.xlsx          # BEA GDP-by-Industry gross output (downloaded by download_data.py)
│   ├── 2-6 digit_2017_Codes.xlsx # Census NAICS code crosswalk (downloaded by download_data.py)
│   ├── compustat_fundq.parquet   # Compustat quarterly data (downloaded by download_data.py)
│   ├── compustat_funda.parquet   # Compustat annual data (downloaded by download_data.py)
│   └── qfr/
│       └── QFR-mf.csv            # Census Quarterly Financial Report (downloaded by download_data.py)
├── proc_data/
...
| `[wrds] wrds_user / wrds_password` | `download_data.py` (Compustat download) | Institutional subscription — <https://wrds-www.wharton.upenn.edu/> |
| `[fred] fred_api_key` | R script 10 | Free — register at <https://fred.stlouisfed.org/docs/api/api_key.html> |

The BEA and Census data files are downloaded automatically by `code/download_data.py` — no key required.
...
This downloads BEA NIPA tables, GDP-by-Industry gross output, the Census NAICS crosswalk, Census QFR data, and (if WRDS credentials are configured) Compustat quarterly and annual fundamentals into `raw_data/`. Files are only downloaded if they don't already exist. Re-run after deleting files to pick up updated releases.
...
| `02_aggregates.R` | BEA aggregate price, cost, and profit series | BEA downloads (`download_data.py`) | `aggregates.pdf` |
...
| `07_qfr.R` | Census QFR profit margins by sector | QFR download (`download_data.py`) | `replicate_plot.pdf`, `qfr_profits.pdf` |
...
| [Bureau of Economic Analysis (BEA)](https://www.bea.gov/) | NIPA Tables 1.15 & 6.16D, GDP-by-Industry gross output | Public download via `download_data.py` — no key required |
| [Census Bureau](https://www.census.gov/) | NAICS 2017 code crosswalk | Public download via `download_data.py` — no key required |
| [Census Bureau QFR](https://www.census.gov/programs-surveys/qfr.html) | Quarterly Financial Report | Public download via `download_data.py` — no key required |
| [WRDS / Compustat](https://wrds-www.wharton.upenn.edu/) | Firm-level quarterly and annual financials | Institutional subscription; downloaded via `download_data.py` to local parquet files |
